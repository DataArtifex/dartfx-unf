...
The objective of this project is to implement a Universal Number Fingerprint (UNF) v6 calculator in Python based on the UNF v6 specification document from the [DataVerse Guide](https://guides.dataverse.org/en/latest/developers/unf/unf-v6.html) website.

A local copy is available in the [UNF_V6.md](UNF_V6.md) file.

The focus is on text delimited and parquet data files formats.

The package must be able to process very large data files exceeding the available memory.

Performance is critical.

Use the Python Polars package for implementation.

The design must take into account that the package can be used as a library or as a command line tool, as well as easy to integrate in APIs or MCP servers.
...
- The source of truth for the version is located in: `src/dartfx/unf/__about__.py`.
