...
├── unf.schema.json   # Bundled JSON schema for output validation
...
5. **JSON report schema.** The `UNFReport` object serializes to JSON matching the bundled `unf.schema.json`, providing full traceability from dataset → file → column.
...
| — | JSON report | Structured output matching `unf.schema.json` with schema validation. |
