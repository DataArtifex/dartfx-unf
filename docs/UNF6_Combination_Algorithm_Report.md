...
A shared, language-agnostic test suite (e.g., a JSON file with inputs and expected outputs at each level — normalization, column, file, dataset) would greatly facilitate cross-validation between implementations. The `unf6_schema.json` schema is a good foundation for this.
